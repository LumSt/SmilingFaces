...
class ViewController: UIViewController {
    ...
    @IBAction func onTrayPanGesture(_ sender: UIPanGestureRecognizer) {
        ...
        if velocity.y > 0 {
            ...
            arrayImageView.transform = CGAffineTransform.identity.rotated(by: CGFloat(M_PI))
                //CGAffineTransform(rotationAngle: 360.0 * CGFloat(M_PI) / 360.0)
                //arrayImageView.transform.rotated(by: 360.0 * CGFloat(M_PI) / 360.0)
        } else {
